...
4. 제스처 4종 (프로토에서 검증 완료): 탭=찌르기(부위 판정: 머리/배/엉덩이), 길게=누르기(바닥으로 찌부러짐), 몸통 빠른 문지르기=간지럼(참기→빵 터짐 2단계), 정수리 문지르기=쓰다듬기(게이지 감소), 가로 휙=밀어 넘어뜨리기(버둥→탭 도움/방치 분기)
5. 삐짐 중 문지르기=쓰다듬기(화해 진입)
6. 빡침 게이지 (UI 포함)
7. PWA 기본 골격 (Vercel 배포 가능 상태)

### 검증된 판정/모션 수치 (프로토에서 확정, 변경 금지)
- 누르기 발동: 380ms 홀드 (이동 시 취소)
- 밀기: 가로 이동 >90px, 450ms 이내, |dx| > |dy|×1.3
- 간지럼: 에너지 누적식(이동속도 합산, 200ms마다 ×0.8 감쇠), 참기 진입 140, 빵 터짐 460
- 덩치감 모션: squash 0.45s / 출렁 1.5s / 숨쉬기 2.9s — 더 빠르게 하지 말 것 (작아 보임)
- 게이지: 찌르기 +6, 누르기 +16, 간지럼 +4, 방치 기상 +20, 도움 -15, 쓰다듬기 -9, 초당 -1.5 자연 감소
